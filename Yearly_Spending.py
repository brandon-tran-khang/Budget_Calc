...
# New: Category Mapping for consistency
CATEGORY_MAPPING = {
...
def load_and_combine_csv_files(directory):
    ...
    # Look for Chase and Citi (Year to date) files
    ...
    for file in files_found:
        try:
            if "Year to date" in file.name:
                # Citi files have a metadata header on line 1
                df = pd.read_csv(file, skiprows=1)
                # Normalize Citi columns to match Chase format
                ...
                print(f"Loaded and normalized Citi: {file.name}")
            else:
                df = pd.read_csv(file)
...
def process_transactions(df, year=2026):
    ...
    # FIX: Use format='mixed' to handle "01/22/2026" and "Jan 22, 2026" simultaneously
...
def main():
    # --- Cleanup Section ---
    # List of files to remove from the Data folder if they exist
    files_to_remove = [
    ...
    # Summaries (keeping index for consistency)
    ...
    print(f"✓ Success! Processed {len(df_spending)} spending transactions.") 
